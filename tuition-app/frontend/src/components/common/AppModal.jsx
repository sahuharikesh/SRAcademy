...
export default function AppModal({ open, onClose, title, subtitle, width = 400, destroyOnClose = false, children }) {
  return (
    <Modal
      ...
      styles={{ content: { border: '2px solid var(--brand-gold, #C9A84C)', padding: 0, borderRadius: 16, overflow: 'hidden' } }}
    >
      <div className="px-5 py-4 flex items-start justify-between"
        ...
        <div>
          ...
          {subtitle && <p className="text-xs mt-0.5" style={{ color: 'var(--brand-gold, #C9A84C)' }}>{subtitle}</p>}
        </div>
