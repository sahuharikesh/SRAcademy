...
function PayModal({ fee, onClose, onSuccess }) {
  ...
  const [payment, setPayment] = useState(String(totalDue));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl"
        ...
        <div className="px-5 py-4 flex items-start justify-between"
          ...
          <div>
            <h2 className="text-sm font-black text-white">Record Payment</h2>
            <p className="text-xs mt-0.5" style={{ color: '#C9A84C' }}>
              {fee.studentId?.name} — {fee.month} {fee.year}
            </p>
          </div>
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          ...
          <div className="flex justify-between text-sm border-t pt-2">
            ...
            <span className="font-black text-base" style={{ color: '#dc2626' }}>₹{totalDue}</span>
          </div>
          ...
          <div className="mt-1">
            ...
            {Number(payment) > 0 && Number(payment) < remaining && (
              <p className="text-xs mt-1" style={{ color: '#b45309' }}>
                Still ₹{remaining - Number(payment)} pending on this month — will be marked Partial
              </p>
            )}
...
function ConfirmModal({ message, onConfirm, onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl"
        style={{ background: '#fff', border: '2px solid #C9A84C' }}>
        <div className="px-5 py-5 text-sm text-center" style={{ color: '#1a1a1a' }}>
          {message}
        </div>
        <div className="px-5 pb-5 flex gap-2">
          <button onClick={onClose}
...
export default function Fees() {
  ...
  return (
    <div className="anim-fade-up">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setShowAddFee((v) => !v)}
            className="btn-shine px-4 py-2 rounded-lg text-sm font-bold" style={GOLD}>
            + Add Fee
          </button>
          <button onClick={handleGenerate}
            className="btn-shine px-4 py-2 rounded-lg text-sm font-bold" style={DARK}>
      ...
      {/* Pay Modal */}
      {payModal && (
        <PayModal fee={payModal} onClose={() => setPayModal(null)} onSuccess={load} />
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
          ...
          <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl"
            ...
            <div className="px-5 py-4 flex items-start justify-between"
              style={{ background: 'linear-gradient(135deg,#1a1a1a,#2a2a2a)', borderBottom: '2px solid #C9A84C' }}>
              <div>
                <h2 className="text-sm font-black text-white">Send Fee Reminder</h2>
                <p className="text-xs mt-0.5" style={{ color: '#C9A84C' }}>
                  {waModal.fee.studentId?.name} — {waModal.fee.month} {waModal.fee.year}
                </p>
              </div>
              <button onClick={() => setWaModal(null)} className="text-gray-400 hover:text-white text-xl font-bold leading-none">✕</button>
            </div>
            <div className="px-5 py-4 flex flex-col gap-4">
              <div className="flex flex-col items-center gap-2">
                ...
                <img src={waModal.qrUrl} alt="UPI QR"
                  className="w-48 h-48 rounded-xl self-center"
                  style={{ border: '2px solid #C9A84C' }} />
                <div className="text-center">
                  <div className="text-xl font-black" style={{ color: '#1a1a1a' }}>₹{waModal.amount}</div>
            ...
            <div className="px-5 pb-5 flex gap-3">
              ...
              <button onClick={handleSendFromModal}
                className="flex-1 py-2.5 rounded-lg text-sm font-black flex items-center justify-center gap-2"
